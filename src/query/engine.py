...
class RAGEngine:
    ...
    async def warmup_models(self) -> dict[str, bool]:
        """预加载 embedding + reranker 到 GPU。

        幂等（已加载则跳过）、带 GPU 检测（无 CUDA 自动 CPU 降级）、
        带失败恢复（GPU 加载失败后自动 fallback 到 CPU）。

        Returns:
            {"embedding": bool, "reranker": bool} — 各模型加载状态
        """
        status: dict[str, bool] = {"embedding": False, "reranker": False}

        # 检测 GPU 可用性
        gpu_available = torch.cuda.is_available()
        if not gpu_available:
            logger.warning("CUDA not available, models will run on CPU (performance may be reduced)")

        # 加载 embedding 模型
        try:
            _ = self.hybrid_retriever.vector_retriever.embedding_model
            status["embedding"] = True
            logger.info("Embedding model loaded (GPU)" if gpu_available else "Embedding model loaded (CPU)")
        except RuntimeError as e:
            if "out of memory" in str(e).lower() or "cuda" in str(e).lower():
                logger.warning(f"GPU memory error loading embedding model, falling back to CPU: {e}")
                await self._fallback_embedding_to_cpu()
                status["embedding"] = True
            else:
        ...
        # 加载 reranker 模型
        try:
            self.reranker._ensure_model_loaded()
            status["reranker"] = True
            logger.info("Reranker model loaded (GPU)" if gpu_available else "Reranker model loaded (CPU)")
        except RuntimeError as e:
            if "out of memory" in str(e).lower() or "cuda" in str(e).lower():
                logger.warning(f"GPU memory error loading reranker model, falling back to CPU: {e}")
                self.reranker.device = "cpu"
                self.reranker.model = None
                try:
                    self.reranker._ensure_model_loaded()
                    status["reranker"] = True
                except Exception as fallback_err:
                    logger.error(f"Reranker CPU fallback also failed: {fallback_err}")
            else:
                logger.error(f"Reranker model failed to load: {e}")
        except Exception as e:
            logger.error(f"Reranker model failed to load: {e}")
    ...
    async def _fallback_embedding_to_cpu(self) -> None:
        """重新初始化 embedding 模型到 CPU。"""
        from sentence_transformers import SentenceTransformer

        settings = get_settings()
        model_name = settings.models.embedding.name
        self.hybrid_retriever.vector_retriever._embedding_model = SentenceTransformer(
            model_name,
            device="cpu",
            model_kwargs={"torch_dtype": torch.float16},
        )
        logger.info(f"Embedding model re-initialized on CPU: {model_name}")
    ...
    async def query_stream(
        ...
        try:
            ...
            yield {
                ...
                "data": {
                    "session_id": request.session_id or "",
                    "trace_id": trace_id or request_id_var.get(),
                },
            }
