...
class LLMGenerator:
    ...
    def _ensure_client(self) -> None:
        ...
        if LLMGenerator._client is None or LLMGenerator._client_config != config_key:
            LLMGenerator._client = AsyncOpenAI(
                ...
                timeout=120.0,
                max_retries=3,  # ponytail: retry on transient failures
            )
    ...
    async def generate(
        ...
        # ponytail: cache LLM responses for identical (query, node_ids) pairs
        if not conversation_history:
            node_ids = tuple(n.node_id for n in contexts[:5])
            cache_key = make_cache_key("llm", query, node_ids)
        ...
        response = await self._call_with_retry(messages=messages)
        ...
        # ponytail: only cache single-turn responses (no conversation history)
        if not conversation_history:
            await cache_mgr.set(cache_key, result, ttl=1800)
