...
class SessionManager:
    ...
    def __init__(
        ...
        self.cache_manager = cache_manager  # Redis 缓存（可选）
    ...
    async def _cache_session(self, session: ConversationSession) -> None:
        ...
        try:
            ...
            await self.cache_manager.set(
                f"{SESSION_CACHE_PREFIX}:{session.session_id}",
                data,
                ttl=SESSION_CACHE_TTL,
            )
        except Exception as e:
            logger.warning(f"Failed to cache session {session.session_id}: {e}")

    async def _get_cached_session(self, session_id: str) -> ConversationSession | None:
        """从 Redis 读取会话缓存。"""
        if not self.cache_manager:
            return None
        try:
            key = f"{SESSION_CACHE_PREFIX}:{session_id}"
            data = await self.cache_manager.get(key)
            if data:
                return ConversationSession(**data)
        except Exception as e:
            logger.warning(f"Failed to read cached session {session_id}: {e}")
        return None
    ...
    async def _delete_cached_session(self, session_id: str) -> None:
        ...
        if not self.cache_manager:
            return
        try:
            await self.cache_manager.delete(f"{SESSION_CACHE_PREFIX}:{session_id}")
        except Exception as e:
            logger.warning(f"Failed to delete cached session {session_id}: {e}")
    ...
    def create_session(self) -> ConversationSession:
        ...
        try:
            asyncio.ensure_future(self._cache_session(session))
        except RuntimeError:
            pass  # no event loop
        return session
    ...
    async def create_session_db(self) -> ConversationSession:
        ...
        await self._cache_session(conversation_session)
        return conversation_session

    async def get_session(self, session_id: str) -> ConversationSession | None:
        """Get session: memory → Redis cache."""
        # ponytail: sync check from memory first
    ...
    def get_session_sync(self, session_id: str) -> ConversationSession | None:
        """同步版: 仅从内存查询, 不涉及 IO。"""
    ...
    async def get_or_load_session(self, session_id: str) -> ConversationSession | None:
        ...
        Falls back through: memory → Redis cache → PostgreSQL.
        """
        # 1) 内存
        session = self.sessions.get(session_id)
        if session:
        ...
        if cached:
            self.sessions[session_id] = cached
            if not cached.messages:
                await self._load_messages_if_needed(session_id)
            return self.sessions.get(session_id)

        # 3) 后备：返回 None，由调用方决定是否从 PG 加载
        return None
    ...
    async def add_message(
        ...
        # 更新 Redis 缓存
        await self._cache_session(session_obj)
