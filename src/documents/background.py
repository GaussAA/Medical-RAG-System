...
async def process_batch_documents_background(
    ...
    try:
        ...
        # Step 2: Save headings via DocumentStore (delegates to store.save_headings)
        doc_heading_ids: dict[str, dict[int, str]] = {}
        for doc_id, (_chunks, heading_tree, _nodes) in doc_chunks_map.items():
            try:
                doc_store = DocumentStore()
                doc_heading_ids[doc_id] = await doc_store.save_headings(doc_id, heading_tree)
            ...
            for doc_id, (chunks, heading_tree, retrieved_nodes) in doc_chunks_map.items():
                try:
                    ...
                    if not chunks:
                        ...
                        # Delegate chunk saving to DocumentStore
                        doc_store = DocumentStore(async_session=session)
                        await doc_store.save_chunks(doc_id, chunks, heading_ids_map)
