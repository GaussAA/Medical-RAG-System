...
# ponytail: cache document list for 10s to avoid re-fetch on every interaction
@st.cache_data(ttl=10)
def fetch_documents(page: int, page_size: int) -> dict:
    try:
        r = requests.get(
        ...
        return r.json() if r.status_code == 200 else {"documents": [], "total": 0}
    except Exception:
