...
# Add src/ to Python path for imports from the new architecture
...
# Quick stats — cached because they change infrequently
@st.cache_data(ttl=30)
def _fetch_doc_count() -> str:
    try:
        r = requests.get("http://localhost:8000/api/v1/documents?page_size=1", timeout=5)
        return str(r.json().get("total", "N/A")) if r.status_code == 200 else "N/A"
    except Exception:
        return "N/A"

@st.cache_data(ttl=30)
def _fetch_session_count() -> str:
    try:
        r = requests.get("http://localhost:8000/api/v1/sessions", timeout=5)
        return str(len(r.json())) if r.status_code == 200 else "N/A"
    except Exception:
        return "N/A"

col1, col2, col3 = st.columns(3)
col1.metric("已上传文档", _fetch_doc_count())
col2.metric("对话会话", _fetch_session_count())
