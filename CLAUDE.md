...
# 环境初始化
uv sync                                           # 安装依赖
uv run python scripts/init_db.py                  # 初始化 PostgreSQL（首次运行前）
uv run python scripts/init_vector_db.py            # 初始化 Qdrant（首次运行前）
...
uv run pytest tests/integration/                   # 集成测试（需 PostgreSQL + Qdrant 运行）
uv run pytest tests/unit/test_vector_retriever.py -v  # 运行单个测试文件

# 流式查询 API
curl -X POST "http://localhost:8000/api/v1/query/stream" \
  -H "Content-Type: application/json" \
  -d '{"question": "黑色素瘤的危险因素有哪些？"}'
```
