...
async def run_full_evaluation():
    ...
    with open("data/evaluation_dataset.jsonl", "r", encoding="utf-8") as f:
        ...
        for i, item in enumerate(queries):
            ...
            try:
                ...
                if citations and answer:
                    ...
                    inability_phrases = ["无法回答", "未包含", "无法找到", "无相关信息", "没有相关信息", "未找到相关内容", "无法完全回答"]
                    ...
                    if correctly_identifies_gap and verified_citations:
                        ...
                        faithfulness = min(0.3 + (len(verified_citations) / max(len(citations), 1)) * 0.7, 1.0)

                answer_relevancy = 0.5
                if answer:
                    query_terms = set(query_text.lower().split()) - {"的", "了", "是", "在"}
                ...
                print(f"  [OK] Overall: {overall_score:.3f} | Faithfulness: {faithfulness:.3f} | "
                      f"Hallucination: {hallucination_ratio:.2%} | Citations: {len(citations)}(v:{len(verified_citations)})")

            except httpx.TimeoutException:
                print(f"  [TIMEOUT] Timeout after 120s")
                stats["failed"] += 1
    ...
    avg_faithfulness = sum(stats["faithfulness_scores"]) / len(stats["faithfulness_scores"]) if stats["faithfulness_scores"] else 0
    avg_hallucination = sum(stats["hallucination_ratios"]) / len(stats["hallucination_ratios"]) if stats["hallucination_ratios"] else 0
